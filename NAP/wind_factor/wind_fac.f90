program wind_factor
  ...
  character*64 :: s_ddir,s_wdir,o_wdir,o_fdir
  ...
  s_ddir='/Users/yaguchi/HDD2/out/NAP_area/drift/'
  s_wdir='/Users/yaguchi/HDD2/data/wind_north/moor_area/'//trim(adjustl(fmonth))//'/'
  o_wdir='/Users/yaguchi/HDD2/out/wind/NAP_area/'
  o_fdir='/Users/yaguchi/HDD2/out/NAP_area/wind_fac/'
  
  dfile1=trim(adjustl(s_ddir))//trim(adjustl(tyear))//'/cul_file1.txt'
  dfile2=trim(adjustl(s_ddir))//trim(adjustl(tyear))//'/cul_file2.txt'

  open(11,file=dfile1,status='old')
  open(21,file=dfile2,status='old')

  open(33,file=trim(adjustl(o_fdir))//'seta.txt',status='replace')
  open(66,file=trim(adjustl(o_fdir))//trim(adjustl(fmonth))//'.txt',status='replace')
  open(77,file=trim(adjustl(o_fdir))//'winm.txt',status='replace')
  open(88,file=trim(adjustl(o_fdir))//'ave.txt',status='replace')
  open(99,file=trim(adjustl(o_wdir))//'win_ave.txt',status='replace')
...
!  stop
 ...
 !    print *,d1
     ...
     open(70,file=trim(adjustl(s_ddir))//trim(adjustl(tyear))//'/' &
     ...
     if ((hh1.ge.0).and.(hh1.lt.6)) then
        wfile=fy//mon//nd//'00_win.dat'
     endif

     if ((hh1.ge.6).and.(hh1.lt.12)) then
        wfile=fy//mon//nd//'06_win.dat'
     endif

     if ((hh1.ge.12).and.(hh1.lt.18)) then
        wfile=fy//mon//nd//'12_win.dat'
     endif

     if ((hh1.ge.18).and.(hh1.lt.24)) then
        wfile=fy//mon//nd//'18_win.dat'
     endif

     open(71,file=trim(adjustl(s_wdir))//wfile,recl=4,status='old',form='unformatted',access='direct')
...
!     enddo
  ...
  do j=1,gy
     do i=1,gx
        ...
        svel=sqrt(xmu**2+xmv**2)
        write(99,*) xmu,xmv,svel,ymu,ymv
